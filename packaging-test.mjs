...
console.log('\n[D] 真打包 + 解包后隐私自查（个人绝对路径 0 命中）')
{
  ...
  try {
    const out = execFileSync('npm', ['pack', '--pack-destination', tmp, '--json'], { cwd: HERE, encoding: 'utf8', timeout: 180000 })
...
{
  let rc = 0, out = ''
  try { out = execFileSync('bash', ['start-demo.sh', '--check'], { cwd: HERE, encoding: 'utf8', timeout: 60000 }) } catch (e) { rc = e.status; out = String(e.stdout || '') }
  check('--check 预检通过（rc=0）', rc === 0, out.trim().split('\n').pop())
  check('--check 打印了包解析器来源', /包解析器/.test(out))
  let rc2 = 0
  try { execFileSync('bash', ['start-demo.sh', '--check', '--port', 'abc'], { cwd: HERE, encoding: 'utf8', timeout: 30000 }) } catch (e) { rc2 = e.status }
  check('非法端口 → 退出码 2', rc2 === 2, String(rc2))
  let rc3 = 0, help = ''
  try { help = execFileSync('bash', ['start-demo.sh', '--help'], { cwd: HERE, encoding: 'utf8', timeout: 30000 }) } catch (e) { rc3 = e.status }
...
{
  let rc = 0, out = ''
  try { out = execFileSync('bash', ['check.sh', '--no-gate', '--json'], { cwd: HERE, encoding: 'utf8', timeout: 180000 }) } catch (e) { rc = e.status; out = String(e.stdout || '') + String(e.stderr || '') }
...
console.log('\n[G] 新增文件卫生：无个人路径 / 不误纳本机数据')
{
  ...
  const mine = ['we-scene.mjs', 'start-demo.sh', 'check.sh', 'sw.js', 'sw-policy.mjs', 'pwa-inject.mjs', 'make-icons.mjs', 'manifest.webmanifest', 'mount-test.mjs', 'pwa-test.mjs', 'packaging-test.mjs', 'hlsl2glsl-coverage-test.mjs']
  for (const f of mine) {
    const s = RD(f)
  ...
  check('新增文档 PACKAGING.md 存在且被 files 收录', fs.existsSync(path.join(HERE, 'PACKAGING.md')) && ((pkg && pkg.files) || []).includes('PACKAGING.md'))
}
