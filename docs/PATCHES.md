...
**语料计数（本轮实测，38 个容器）**：`allwallpaper/`（21 个 scene.pkg）+ 插件缓存 `~/.dsh-mpkg-wallpaper`
（11 个 mpkg）+ `$MPW_SD_ROOT/wallpapertest1`（6 个）⇒ **非正交包恰好 1 个 = `3509243656`**；
...
## P-136（2026-09-19 用户第 4 项）鼠标尾迹：**照抄上游 oneincase/webwallgl（MIT）指针通路** + 修「指针进粒子缓存签名 ⇒ 每帧从 t=0 重放 ⇒ 尾迹被抹平」

> **性质**：只读取证 + 定点修复 + **纪律变更（允许复制上游代码）**。全程未开浏览器、未跑全量门禁；证据 = mock-GL 顶点流 + 真包数字 + 与上游 `ParticleSystem` 同参对拍。
> **编号**：落盘前 `grep -n '^## P-13' docs/PATCHES.md` 实测最高 = P-135 ⇒ 用 **P-136**。

### P-136.0 用户原话与纪律变更

> 「你直接把 oneincase 跟鼠标尾迹有关的代码，你看看直接复制过来就算了 —— 你给的这几张图片看不到实质性的内容」

⇒ 要的是**照抄上游实现**，不是"按行为契约自己重写"。本仓库此前对上游的纪律是"只引行为结论、不复制代码"
（`THIRD-PARTY.md` §8/§10）；本节按该直接指令改为**允许复制**，登记 `THIRD-PARTY.md` §14 + `docs/COPYING-RULES.md` §4 #12
（继 §6 FXAA / §9 HLSL→GLSL 之后**第三处逐字例外**）。
边界：**不**复制进 `packages/we-core/`（那个包按 MIT 独立分发，纪律不同）；不复制与鼠标尾迹无关的大段代码。

### P-136.1 根因（一句话）

`core/we-scene-bundle.js` 的粒子缓存签名 `__sig` 里带了**指针坐标**（P-69 起）⇒ 指针一动签名就变 ⇒ **每帧**整系统从 t=0
重放 400 步、且重放全程只用**当前**一个坐标 ⇒ "光标走过的路径"从不进入粒子历史 ⇒ 花瓣永远糊在光标上。
上游相反：指针是**每帧推进的活输入**（建实例后每帧 `advance()` 前调 `setPointer`），指针**从不参与**构造 ⇒ 系统只在时间轴上增量前进，路径留在粒子坐标里。

### P-136.2 照抄清单（上游 file:line → 我们的落点）

| # | 上游单元（checkout `b61e891`） | 落到我们的 | 处置 |
|---|---|---|---|
| 1 | 上游 renderer/vendor/we-scene/render/ 下的 pointer.js 整文件（320 行） | **新** `core/we-pointer-source.mjs` | **正文逐字节相同**（门禁断言 `tail === up`） |
| 2 | `particles.js` 的 `syncLayerTransform`（originX/Y/Z、scaleX/Y、angleZ） | **新** `core/we-particle-pointer.mjs` | 逐字，**仅 angleZ 单位**例外（适配 A-1） |
| 3 | `particles.js` 的 `setPointer` | 同上 | 逐字（`this.`→`sys.`、字段改名，适配 B-1） |
| 4 | `particles.js` 的 `mapAround` 块 | 同上 | 位置段逐字；**初速段未照抄**（适配 C-1，走我们既有 RNG 口径） |
| 5 | `particles.js` 的 `vortex` 块 | 同上 | 逐字（`p.vx+=` → 返回值）；音频门控留在算子层 |
| 6 | `particles.js` 的 `_cpPos` | 同上 | 逐字（适配 B-2） |
| 7 | `renderer/src/scene-mount.ts` 建实例时创建**单一**指针源 | `core/we-scene-bundle.js`（`__ptrSource`） | 接线照抄；`target` 传假对象关掉上游 DOM 监听（适配 D-1） |
| 8 | `scene-mount.ts` **每帧** `advance()` 前 `setPointer(wx,py)`，指针**不进**构造 | `pushPointerFrame(sys,__ptrNow)` + `__sig` 该项改成常量 `'x',` | 语义照抄 —— **这一条就是尾迹的成因修复** |
| — | `packages/we-core/` | — | **未放入任何上游代码**（门禁断言） |

适配 6 处（A-1 角度单位、B-1/B-2 字段改名、C-1 RNG 源、D-1 DOM 监听策略、E-1 半径权重口径）逐行写在 `THIRD-PARTY.md` §14.3。

### P-136.3 真包数字（`dd/3554161528` `objects[27]`=id389 Cherry_Blossoms_2.json，指针 40px/帧 × 30 帧）

| 指标 | 修前 | 修后 | 上游同参 |
|---|---|---|---|
| 顶点流世界包围盒 x 跨度 | 67.4 px | **1175.2 px** | 1112 px |
| 距指针最远 | 39.6 px | **1164.3 px** | 1112 px |
| 存活粒子数 | 163 | 163 | 161 |
| 速度 p50 / p95 / max | 99.8 / 129.0 / 140.1 | 193.6 / 221.2 / 233.4 | 153.8 / 215.0 / 218.3 |
| 每帧仿真步数 / 30 帧内重建 | **400 / 29** | **1 / 0** | — |
| 每帧粒子更新 | ≈61,000 | 0（增量） | — |
| 顶点流 u 跨度 | 1/13 | 1/13（未回归） | — |

### P-136.4 保留的既有契约（不回归）

- 指针为 null（`pointerleave` / `pointerout` / blur / `visibilitychange` / `inside:false` / `?cursor=off`）⇒ **显式清空该层粒子**
  （旧口径靠"重建"实现这个副作用；照抄后指针不再进签名 ⇒ 改成显式规则），救回 `tests/pointer-leave-test.mjs` 的
  P3b/D1b/G5/G1/G1b/G2b/G2c/G3a/G3c/G3e 十条 + P4c。
- **A3d 按主对话批准收窄口径**：「全部存活粒子在 6px 内」→「**本帧新生**粒子在 6px 内」——旧口径 = 禁止滞后粒子 = 禁止尾迹，
  **口径错的是断言不是实现**（它此前靠"每帧重建把粒子冲掉"这个副作用才绿）。同台新增 A3d2 正向钉住"尾迹确实存在"
  （全部最远 / 新生最远 = 437×），并补变异 M9（发射基准点退回 authored origin）保证 A3d 仍可红。

### P-136.5 门禁

`tests/pointer-trail-copy-test.mjs`：**44 断言**（秒级 / 无浏览器 / 无网络 / 真包缺失 SKIP+exit 0）——
①逐字节完整性 + 照抄 token 同一式子 + `packages/we-core` 未污染；②照抄单元的纯函数数字；
③真包尾迹数字 + 上游同参对拍 + 尾迹存在正向判据；④**RED-IF-REVERTED**（指针写回签名 ⇒ 子进程 rc=1 且
`spreadX=67 / rebuilds=29 / simSteps=400` 回到旧口径；并带"未变异真文件 rc=0"反向自证）。

### P-136.6 未证实 / 需要人眼

1. **真机观感未验**（唯一硬缺口）：本批全部证据是 mock-GL 顶点流与数值。真机请用
   `?id=3554161528`（或 `&ln=389` 单看该层）**匀速画圈/横移 ≥1 秒**，期望光标**身后**拖出约 1000+ px 的彗尾。
2. `spritetrail` 的几何拉伸仍被 authored `maxlength:1` 夹住（quad 仍是方块）——本层尾迹来自**粒子滞后**而非拉丝；
   若真机仍嫌"没有拉丝感"，那是另一条（未做）。
3. **发现但未修（不在照抄范围、会改全语料）**：我们 `sizechange`/`alphachange` 的 `endvalue` 缺省是 **0**，上游是 **1**
   ⇒ 上游花瓣整段寿命保持 30–40px，我们同一时刻 size p50=21.4 / min=0.24（尾段被缩没）。要改需单独立项（149 个语料系统一起变）。
...
> 口径说明（主对话补注）：本条**改的是 `dsh-mpkg-wallpaper`（插件仓）**，代码提交 `d072858`+`d3eea63`+`2185fc2`；
> 它出现在**渲染器仓**的台账里，是因为 P-152 那条线提交 `docs/PATCHES.md` 时把工作区里已写好的这一段一并带进来了。
> 编号本身不冲突（本仓 P-149/P-152/P-153 与它各自唯一、非递减），**故意保留不清除** —— 证据留痕比"台账好看"重要。
...
## P-156（2026-09-19 · **插件侧** NP-3，不是渲染器侧）Now playing / 壁纸声音：一批真机 bug 的根因与修法

> 口径说明：与 P-155 同一个道理 —— 本条**改的是 `dsh-mpkg-wallpaper`（插件仓）**，登记在渲染器仓的台账里
> 只是因为 P 编号台账在 `we-scene-demo/docs/PATCHES.md` 这个文件里（跨仓先例见 P-155 的口径补注 `fcf16c1`）。
...
2. **音轨清单路由**：自定义目录的壁纸不写 `folderName`（web 是 `mpkgKey="custom|<folder>"`）⇒
   旧实现拼出 `library-scene-audio?ltoken=custom|…`，宿主 404 ⇒ 目录里的音频永远接不上。
3. **apply 路径**：`applyFromStorageInner()` 的 **web 分支提前 `return`**，把共享尾里的
   `applyNowPlaying()`（以及 `data-mpw-float` 门控）整段跳过 ⇒ 刷新/切到 web 壁纸后控件根本不挂。
4. **CSS 产物**：`buildCss()` 的 `if (!hasImage)` 早退漏拼 NP 段 ⇒ 没有 `image`/`webUrl` 的档
   整份 NP 样式丢失（`.mpw_np_*` 全变 static，传输键掉到卡片外点不到）。
5. **布局量宽**：贴合缩放量"侧栏列"而真机上 `[class*="sidebarCol"]` 不止一处（量到 280，容器只有 256），
   且 `.mpw_np` 没有确定宽度（grid 把 `width:auto` 夹成 232）⇒ `scale()` 原点偏右 14px、
   卡片右缘被切 **10.9px**（悬浮效果下最明显）。
6. **标记语义**：播放/暂停那两个四边形由**展开进度**驱动（`tq = 1 - p`）而不是播放状态 ⇒
   收起态恒画播放三角、展开态恒画暂停双条，两种状态各有一半时间在说谎。

顺带三条"根本没接线"：`video.muted` 从建 DOM 起写死 `true`（用户那张 mp4 实测 `ffprobe` 是
h264 + **aac 音轨**，声音一直都在）；静音键只写设置不落元素；宿主侧栏/slot 晚渲染时旧写法打一行
warn 就放弃（不装观察者、不重试）。新要求两条也一并落地：**开关默认改开**（`DEFAULT_NP_NOW_PLAYING=true`）
＋"宿主同一位置已有别的插件注入的元素就**自动让位**"（不挂/撤下、不重建、留
`data-mpw-np-yield="<原因>"` 与一行可读 warn，挂载前 + 挂载后都判）。

### P-156.1 判据（无浏览器 + 真机探针）

* **新增 `dsh-mpkg-wallpaper/tools/np-media-test.mjs`：82 通过 / 0 失败**（10 组断言 + **12 组变异自证**，
  ~7s，无浏览器）：作用域与宿主路由 / 数据源判定 / 播放与换曲落点 / 静音落点 / 让位夹具 /
  标记状态 / web 路径不许跳过 apply / 卡片几何（宽度 = `math.W` 且溢出均分、fit 按容器实测宽）。
* **既有 `tools/now-playing-test.mjs`：83 通过 / 0 失败**（原 68 条 + 新增；`--no-mutations` 76/0），
  变异 6 → 7 组。改语义的既有断言逐条写在插件仓文档 §7.7.2（默认值 true、关档夹具显式化、
  贴合缩放改成量容器、单实例守卫两处都要打掉才复现）——**不是放宽，断言只增不减**。
* **新真机探针 `tools/np-media-live-probe.mjs`（45 条判据）**：自签 Cookie + headless Firefox 打开 `:3080`；
  修前 **16 PASS / 22 FAIL** → 修后 **45 PASS / 0 FAIL**。真读数举例：`mozHasAudio=true`、
  点一次静音 `mute=false` 且元素 `muted=false`、收起/展开两态 `paused` 真的翻转、
  web 壁纸目录音频 `currentTime=1.81s` 真的推进、上一首/下一首按清单顺序且**全程不 remount**
  （iframe 同一 + src 同 + 0 `loadstart` + 0 mount 信标）、悬浮态卡片四边都在
  容器可视区（`37.03→242.95` vs `24→256`，修前是 `35.03→266.95` ⇒ 越界 `right 10.9px`）、
  让位夹具（先占用 ⇒ 不挂；后插入 ⇒ 撤下且 1.8s 后仍不重建；移走 ⇒ 回来）。
  探针跑前/跑后都要求 `ps -eo comm | grep -cx firefox` 回到 0，峰值内存 ~600MB 级。
* **`tools/check.sh`**：新门禁挂第 2 步（`now-playing-test.mjs` 之后）追加一行，零重编号。

### P-156.2 诚实清单（摘要；全文见插件仓 §7.7.7）

1. web 壁纸**帧内**那份声音只做到静音（播放/暂停需要帧内可达或 shim 扩展，本轮不做）。
2. "壁纸自己也播同一首"会叠音：已做"我们的播放器在放音时强制静音帧内"防护，但**没有真机样例可验**（未证）。
3. 宿主/壁纸自身脚本的 10 条 pageerror（`HTMLScriptElement`/`rawcanvas` 等）修前修后都在、与本插件无关，
   本轮未修；探针判据已收窄到"本插件相关 0 条"。
4. 音量是**静音开关**（既有 `mute` 布尔 + `/media-audio`），不是 0..1 细调。
5. 视频壁纸没有上一首/下一首（单个媒体）⇒ 两键如实 disabled。
6. 卡片里第四个键（音量/静音）只在**展开态**出现：收起态传输行是按 88px 三键行定位的（`opsX(0)=206` 被门禁 C23 钉住）。
7. 用户说的"点上一首会把壁纸视频重载一下"**没能复现**：探针读数显示传输键从未改 iframe src / 无
   `loadstart` / 无 mount 信标（修前也 PASS）。最接近的解释是旧"回到开头"键把视频 `currentTime` 归零、
   画面跳回开头被读成"重载"，但那发生在视频档、用户当时说的是 web 档 —— 如实记为未复现。
8. 用户当前 section 处于"半残档"（`mpkgKey` 在、`image`/`webUrl` 都没了）：本轮只修了"这种档下 NP 样式
...
## P-163（2026-09-19 · 渲染器侧）壁纸 `3669681034`（亚托莉 ATRI 8K）"渲染全黑"：8K free-image 贴图**先解全尺寸再丢 15/16**，以及"上传失败的坏纹理照登记"⇒ 整屏铺不透明黑

**派单假设先被证伪**（现场是 `?id=3669681034`，真语料 `allwallpaper/dd/3669681034/scene.pkg`，94.2MB / 21 条目）：
"全屏黑幕层 + intro 时间轴没跑 ⇒ 永远黑"在这条包里**不成立** —— `scene.json` 只有 **2 个对象**
（1 张整屏底图 + 1 个 sound），容器里**零脚本**（没有 `.js/.lua`，`scene.json` 也没有 `script/scripts` 键），
全文**没有任何 `intro|enter|introanimation|fadein|firstframe` 键**，也没有 `alpha=0` / `color=0,0,0` / `visible=false`
的对象 ⇒ **没有黑幕层可淡出**；sound 对象没有 `image`，按既有语义走"solid 占位 ⇒ 透明兜底"，也当不了黑幕。
投影也不是嫌疑人：`general.orthogonalprojection = 7680×4320`，底图 `objects[0].origin/size = 3840,2160 / 7680,4320`
（JSON 路径见门禁的 A5），正交档 `view = 单位阵`（`buildCamera` 的 `isOrtho` 分支不看 `camera.eye/center`），
门禁用 **mock-GL 抓真顶点流 + 该 draw 自己的 `u_MVP`** 复算，四角 NDC 恰好 `[-1,1]²`（A8）。

**根因（两处，都在贴图装载链上；`file:line` 为改动前快照）**

| # | 落点 | 事实 | 后果 |
|---|---|---|---|
| ① | `core/we-scene-bundle.js:813 decodeMip0()`（旧快照；恒取 `images[0][0]`）+ `demo.html:1835`（旧快照）PNG 分支 | 该贴图 `materials/4k-16-9origin_waifu2x_2x_jpg.tex` 是 **5 级独立 PNG**（7680×4320 / 3840×2160 / 1920×1080 / 960×540 / 480×270，mip0 载荷 **33,288,714B**），而上传目标由 `texDownsampleCap(7680,4320,*)` 定成 **2048** ⇒ 旧链路把 8K 全解成 **132.7MB** 位图，紧接着又缩到 2048×1152（9.4MB）**丢掉 15/16 像素**；容器自带的 4 级更小 mip **一级都没用** | 首帧多花解 8K 的时间与峰值内存（本机实测 `纹理加载完成` **791ms**、导航→首帧 **3945ms**）；手机/WebView 上正是 P-36 记录过的"大位图上传失败 ⇒ 整层采样为黑"那类故障的入口 |
| ② | `demo.html:1904`（旧快照） | 位图上传**重试完仍报错**时只写一行 `⚠ …该层可能采样为黑`，**照旧 `textures.set(name, entry)`** 登记这张不完整纹理 —— 而 WebGL 对不完整纹理采样恒为 `(0,0,0,1)`，本包底图是**整屏 7680×4320** 层 ⇒ 一帧铺满**不透明黑**，用户看到的就是"整个画面完全是黑色的"（清屏色 0.7 灰被它整块盖住） | 同 ① 的设备侧失败会**静默变成整屏黑**，且日志里没有"层没画"的结论 |
| ③ | `demo.html:1892`（旧快照） | 上传失败后的 2048→1024 重试阶梯是**死代码**：`k = min(retry/w, retry/h)` 里 `w/h` 已被 cap 裁过，`retry=2048` 对 2048×1152 恒得 `k=1` ⇒ `k >= 1` 直接 `break`，1024 那档永远到不了 | 唯一可能救回这张贴图的降档重试**从未发生** |

**修法**（`core/we-scene-bundle.js` + `demo.html`，均为最小面；门禁 `scene-intro-black` 29 断言 + 2 组真变异）

1. 新增纯函数 `pickMipForTarget(tex, targetMaxSide)`（`core/we-scene-bundle.js:832`）= 缩放的标准 LOD 选择：
   取**长边 ≥ 目标的最小一级**；target ≤0/非法/比 mip0 还大/只有一级 ⇒ **0（= mip0，逐位回到旧行为）**。
2. 新增 `decodeImageMip(tex, idx, mipLevel)`（`core/we-scene-bundle.js:852`），并把 `decodeImageMip0`（`:863`）加可选第 3 参（缺省 0 ⇒ 逐位不变；
   裁剪到声明尺寸只对"槽 0 的 0 级"成立）。
3. `demo.html` 的 `loadTex` 在 PNG/JPEG 分支**先选级再解码**：`?mipsel=0` 可强制回退做 A/B；
   并把"按 **mip0** 算出的目标"记进 `__mipTarget` 交给位图那段当 cap 用 —— **上传尺寸与改动前逐位一致**
   （否则 3840 ≤ 4096 会让 `texDownsampleCap` 返回 null，显存反涨 3.5×）。
   切片夹具（`load-timeout-test.mjs`）用受限入参执行本块 ⇒ 一律 `typeof` 守卫、缺 helper 直接跳过（零日志零行为变化）。
4. 上传失败：重试阶梯按**目标上限**逐档缩（2048 → 1024，死代码修好）；全部失败 ⇒ **返回 null 且不登记**
   （该层走既有"缺纹理 ⇒ 透明兜底"路径，画面退回清屏色而不是黑幕，原因写在日志里）。

**修前/修后数值（真包 `3669681034`，`?device MAX_TEXTURE_SIZE=16384`）**

| 指标 | 修前 | 修后 |
|---|---|---|
| 交给浏览器解码的 mip | 第 0 级 **7680×4320 / 33,288,714B** | 第 1 级 **3840×2160 / 9,107,059B**（−72.6% 字节） |
| 解码后峰值位图 | **132.7MB**（7680×4320×4） | **33.2MB**（−75%） |
| 上传尺寸（= 显存） | 2048×1152（9.4MB） | **2048×1152（不变）** |
| 上传恒报 0x501 时 | 登记坏纹理 ⇒ 该层采样 `(0,0,0,1)`，**整屏不透明黑** | 返回 null ⇒ 该层透明兜底 + 一行"按**缺纹理**处理"，不再铺满黑 |
| 真机（同机 headed Firefox + 软件 llvmpipe，1920×1080 画布，`?id=3669681034`） | `纹理加载完成` 791ms · 导航→首帧 3945ms · 画面 mean `[196,184,160]`、近黑 **0%** | `纹理加载完成` **277ms** · 导航→首帧 **3064ms** · 画面 mean `[196,184,160]`、近黑 **0%**（无可见回归） |
| 同机 A/B 对照（同一个二进制，`?mipsel=0` = 旧路径 vs 默认 = 新路径） | `?mipsel=0`：`纹理加载完成` 761ms · 首帧 6480ms · 峰值 RSS 827MB · 日志 `7680x4320 → 2048x1152` | 默认：`纹理加载完成` **277ms** · 首帧 **5800ms** · 峰值 RSS 821MB · 日志 `mip 选级 → 3840x2160` + `3840x2160 → 2048x1152`；两档画面读回逐值相同（mean `[196,184,160]`、近黑 0%、`pageerrors []`） |
| 峰值内存（诚实项） | —— | **没有降**（827 → 821MB，噪声内）：本机浏览器峰值由软件 GL + 浏览器本体主导（`?res=720p` 仍 755MB）。本补丁降的是"解码字节/峰值位图/加载阶段耗时"，**不是**整机 RSS |

**语料同类计数**（门禁 D 段逐包扫目录表 + entry 头；`free-image` = `.tex` 的 `freeImageFormat ∈ {PNG(13), JPEG(2)}`）：
24 张 free-image 贴图 / 10 包；**长边 >4096 的 2 张**（`3669681034` 7680×4320、`3544152633/housebasic` 4948×2935）；
**长边 >2048 的 10 张 / 5 包**（3544152633 ×5、3554161528 ×1、3669681034 ×1、3721991999 ×1、3778592720 ×2）
—— 这 10 张在任何 `降采样上限 ≤2048` 的设备上都走同一条"先解全尺寸再缩"的链，本次修法对它们**一律生效**
（无 mip 链或非 free-image 格式不受影响，逐位不变）。

**判据**：`tests/scene-intro-black-test.mjs`（新，注册进 `tests/run-all-tests.sh` 为 `scene-intro-black`，条件项：
缺真包语料 ⇒ `SKIP`）。**29 断言 / 0 失败，~0.9s**：A 段现场取证与否证（含真顶点流 NDC）、B 段修复语义与真包数值、
B8-B12 用**真源码切片**的 `loadTex` + 真包 + 假 GL/document 跑装载链、C 段**两组真变异**（M1 `pickMipForTarget → 0`
⇒ B3/B8 必红；M2 把"不登记"换回旧的一行 `logf` ⇒ B11 必红）、D 段语料计数。
诊断开关新增 `?mipsel=0`（已登记 `docs/README-DIAGNOSTICS.md` 主表）。
**未证（写清楚）**：本机 Android/PRoot 上 headless Firefox **没有 WebGL2**（实测 `FEATURE_FAILURE_WEBGL_EXHAUSTED_DRIVERS`），
Chromium 在本环境起不来（GPU 进程连崩/`newPage` 挂起，与仓库既有记录一致）⇒ **用户设备上"到底哪一步变成黑"没有直接读数**：
...
3. **`docs-check` 判红**：反引号里的 shell 脚本后缀被当成"被引用文件"（它按扩展名认路径）。改成带通配符的措辞。
