...
> 解析点抓取（当前 **126** 个）。本表主表与其双向比对，任何一侧多出/缺失都会非零退出。
>
...
| `prot` | `legacy` | `official` | **粒子 quad 旋转档位**（P-101①）：`legacy` = `p.rot` 逐帧算但从不进顶点 ⇒ 花瓣/玻璃碎片/hex 光斑全部轴对齐（"僵死的贴片"，14/40 个测试包） | 复现"粒子不转"的旧画面做 A/B | 仅对照用，非可选画风 | core/we-scene-bundle.js:5851 |
| `pquad` | `legacy` | `official` | **粒子 quad 图层变换档位**（P-101②）：`legacy` = 每颗粒子自己的尺寸/朝向不吃图层 `scale`/z 角 ⇒ 尺寸偏差最大 7.06×（萤火虫；30/40 层受影响） | 复现旧尺寸/朝向做 A/B | 仅对照用 | core/we-scene-bundle.js:5868 |
| `pexp` | `legacy` | `official` | **随机 initializer 的 exponent 非线性分布档位**（P-101③）：`legacy` = 完全不读 `exponent` ⇒ 官方的 `pow(u, exponent)` 退化成均匀分布（Hina 落花尺寸均值 60 而非 53.3） | 复现旧分布做 A/B | 仅对照用 | core/we-scene-bundle.js:5884 |
| `pspeed` | `legacy` | `official` | **发射器 `speedmin/speedmax` 初速档位**（P-101④）：`legacy` = 不读这两个字段 ⇒ 只写 speedmin/speedmax 的发射器（Hina/遐蝶 `cherry blossoms on cursor`）粒子**原地不动** | 复现"粒子不动"的旧画面做 A/B | 仅对照用 | core/we-scene-bundle.js:5898 |
