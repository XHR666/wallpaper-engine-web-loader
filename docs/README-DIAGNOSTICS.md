...
> 解析点抓取（当前 **156** 个：含 P-112-BANDGEOM 的 `bandfeed`/`framegeom`、P-131 批D 的 `audioemit` 与插件侧 `lgcss`）。本表主表与其双向比对，任何一侧多出/缺失都会非零退出。
...
| `mdls` | `legacy` | 无（= **校验档**） | **MDLS 骨骼布局校验的回退开关**（P-152，2026-09-19；属渲染器线）：缺省 = 解析 MDLS 逐骨记录后**先校验再用**（判据：`parent ∈ [-1, 骨数)`、材质(id)索引有界、记录 `len` 有界、骨名槽可打印/合法 UTF-8 且 ≤4096B、旋转部分两行单位长（±0.05）、平移有限、声明骨数 ∈ (0,1024]）；任一骨非法 ⇒ **拒绝整个骨架**（`bones=[]` + 计数 + 一行 warn），并在 `mesh.mdlDiag` 留机器可判台账（`layout` = `A`/`not-A`/`refused`、`reason`、`rejectedBones`）。`legacy` = 跳过校验与判定，**逐位回到 P-152 之前的「无校验」旧行为**。⚠ 依据 `docs/UPSTREAM-PORT-PLAN-20260919.md` §4（上游 `oneincase/webwallgl` `be3c246`，MIT：**先按 A 整体解析并校验、任一骨非法才判变长布局、绝不返回残缺骨架** —— 只作**判据**引用，未照抄代码）；本仓落地范围是**只校验不重扫**（布局 B/C 是**合成**样本、真语料 0 命中 ⇒ 不引入未被真数据检验的启发式）。判据/门禁：`tests/mdl-bone-layout-test.mjs`（68 断言 = 合成逐类判定 + 全语料零回归 + 逐位回退 + 3 组变异自证） | 导入别人的包后 `#log` 出现 `[P-152] MDLS bone layout rejected …`（骨架被判非法、模型/蒙皮消失）时，用 `?mdls=legacy` 一秒回到「按旧定步照读」复现旧画面；或做单变量 A/B 判定「是不是本项判据拦下的」 | ⚠ **缺省档只在「记录本身不合法」时才动作** ⇒ **35/35 合法 MDLS（332 骨）逐位不变**（默认档与 `legacy` 档逐字段相同，门禁有断言）；`legacy` 档下变长布局/损坏记录会**静默产出错位骨架**（`parent` 读成 16256、矩阵退化成垃圾）—— 这正是本项要防的病，只作复现用。台账与 warn **只在真拒绝时**才产生（合法语料零输出 ⇒ 日志面也逐位不变） | elysia/we-renderer/puppet.js:751（判定式唯一实现处；`_parseMdl` 是 `demo.html:3442` **实际运行**的那份解析器，故解析点挂在它上面，而不是 `demo.html`/`core/we-scene-bundle.js`——那两个文件本批禁碰） |
...
| `res` | `720p` / `1080p` / `1440p` / `2160p`（`4k` 同义）/ 显式 `WxH` / `auto` / `legacy` / `dpr`（`dpr1`…`dpr5`） | **1080p** | **画布尺寸 + 视频纹理上传上限的档位**（P-68 用户第 20 项）：`1080p`=1920×1080、`1440p`=2560×1440、`2160p`=3840×2160；显式 `WxH` 与命名档同尺寸时归一成该档（`?res=1280x720` ≡ `?res=720p`）、否则成 `custom` 档；`auto` = 按 `innerWidth×min(dpr,2)` 选"≥物理宽的最小档"（夹在 720p–2160p）；**非法值 → 回退 1080p 并在日志/`videoStats.invalid` 里显式标记**。`720p` 与 `legacy` 是**改动前行为逐位兼容档**（节流 33ms、不写 `imageSmoothingQuality`、超限必走 2D canvas）；**`dpr` 是活档位**（2026-09-21）：画布 = **画布的 CSS 显示尺寸 × min(devicePixelRatio, 上限)**（`dpr1`…`dpr5` = 上限 1×…5×，裸 `dpr` = 不设上限用设备 DPR），上限另有两条硬顶：单边 ≤4096、总像素 ≤3840×2160；**显示尺寸 / DPR / 全屏一变就重算**（`ResizeObserver` + `(resolution: Ndppx)` 媒体查询，120ms 防抖），读数写 `window.__mpwLiveRes = {cssW,cssH,dpr,dprCap,width,height,capped,updates,why}`。为什么另立一档：`auto` 的语义（启动时一次、按 `innerWidth` 向上取整到命名档）被 `tests/video-quality-test.mjs:161-165` 逐值钉住，不能改；而"预览面板多小就按多小出图、全屏要重算"需要**活**档位。判据 `tests/bench-renderer-source-test.mjs` B 段 | 想要清晰就 `?res=1440p`（`?res=2160p` 上行带宽 4 倍，见 PATCHES P-68 四档表）；怀疑分辨率回归就用 `?res=720p` 与旧截图对拍 | 画布像素 ×2.25/×4/×9 → 显存与每帧上传字节同步放大；移动 GPU 上 4K 档不现实（本机未测真机 FPS） | demo.html:3702、core/we-scene-bundle.js:4667/4812 |
...
| `mipsel` | `0` | 开（自动选级） | **大贴图按目标边长选 mip 级**（P-163，2026-09-19「3669681034（ATRI 8K）渲染全黑」）。`.tex` 自带完整 mip 链（该包 5 级独立 PNG：7680×4320/3840×2160/1920×1080/960×540/480×270），旧链路 `decodeMip0` 恒取 0 级 ⇒ 先解出 132.7MB 位图、再被 `texDownsampleCap` 缩到 min(2048,设备上限) 上传（白解 15/16）；现在取"长边 ≥ 目标的最小一级"（标准 LOD 选择：画质不劣、解码量与峰值内存按级数下降）。`?mipsel=0` = 强制回到"恒解 0 级"的旧路径 | 怀疑"某张贴图解码/选级出问题"、或要与改动前做逐位 A/B 时；日志里 `ℹ mip 选级 <名>：目标长边 … → 用第 N 级 …` 会写明选了哪一级、省下多少解码字节 | `=0` 只回到旧行为（大图更慢、峰值位图内存 4×）；对**没有 mip 链**或非 free-image（非 PNG/JPEG/视频）格式的贴图无级可选 ⇒ 两档逐位相同 | demo.html:1734、core/we-scene-bundle.js:832/852 |
| `npvoice` | `keep` / `drop` | 不写 = `auto` | **插件侧**：网页壁纸目录里「交互音/角色语音」是否进 Now playing 播放器清单（P-164，2026-09-20「播放器放的是角色语音而不是背景音」）。`auto` = 按**作者源码**分类（引用上下文就近判 + 名字模式 + Live2D <loadJson.json> 对话表）：判为交互音的**排除**、判不准的**保留**（宁可清单多一条，也不擅自丢用户的声音）；`drop` = 判不准的也排除；`keep` = 一条都不排除 | 清单里出现 `1-1.wav`/`voice_*` 这类角色语音、或反过来「背景音被误排除」时；分类表在 `window.__mpwNpAudioClass` 与 `/diag` 的 `np-audio-class` 里可查 | 只影响**清单**：交互音照旧由壁纸自己播（我们只压背景音）；`keep` 是完全回退（行为与加这个开关之前逐字节一致） | dsh-mpkg-wallpaper/lib/client.js 的 `mpwVoiceMode()` / `mpwClassifyWebAudio()` |
| `npaudit` | `0` | 常开（缺省装） | **插件侧音频审计**（P-165，2026-09-20「什么都没动却又响一下」）：hook `HTMLMediaElement.play`、`volume`/`muted` setter、`Audio` 构造、`AudioContext/webkitAudioContext` 构造与 `decodeAudioData`，以及我们自己的 `npPrimePlay`/`npApplyMute`/`npApplyVolume`/`npEnsureAudio`/`npDropAudio`/`npAudioHardReset`；每条记 `{t,kind,who(栈前3帧),el{tag,id,class,isConnected,muted,volume,paused,currentTime,src摘要},visibilityState,np设置}`，写进**有界环形**（≤200 条，`window.__mpwAudioAudit`）。命中"可疑"（**可听播放转变** `!paused && !muted && volume>0` ∨ `isConnected:false` 仍在播 ∨ hidden 期间 play/取消静音）⇒ 立刻把**审计窗口最近 12 条** POST `/diag`（`kind:"audio-audit"`，脱敏截断，节流 5s） | 用户报"间歇出声"而 30s 采样抓不到时：等下一次响，然后直接读 `/root/.dsh/.dsh-mpkg-wallpaper/diag-*.json` 里的 `audio-audit`（含栈与元素身份） | `?npaudit=0` = **完全不装**（一个钩子都不打，零开销）；缺省常开（只是钩子 + 环形数组） | dsh-mpkg-wallpaper/lib/client.js:`mpwInstallAudioAudit()` / `mpwAuditPush()` |
...
| `scriptstore` | `persist` | **legacy**（逐沙箱内存 Map；`?scriptstore=persist` 才开） | **脚本 `localStorage` 的共享持久档**（P-153，方案 `docs/UPSTREAM-PORT-PLAN-20260919.md` §5）：缺省/任何其它值 = 今天的逐位行为（每个脚本沙箱一份 `new Map()`，脚本之间互相看不见、刷新即复位，**不碰宿主存储**）；`persist` = 官方语义（**同一张壁纸的全部脚本共享一份 + 跨会话持久**），后端 = 宿主 `window.localStorage`，键 = **我们自己的命名空间** `mpw.<包id>.<键>`（`mpw.` 前缀 + `?id=`；`LOCATION_GLOBAL` 落 `mpw.__global.`），只读写这个前缀下的键（`clear()` 也只清本命名空间）。配额/隐私模式/不透明源下后端抛错 ⇒ 记一行 `⚠ [P-153]` warn + **整会话退回内存**（脚本不报错）。判定式唯一实现处 = `elysia/scene-scripts.js::scriptStorePersist` | 想让"记住拖拽位置 / 时钟显隐 / 面板状态"的包（语料 8 个包 66 次调用，如 3326873240 的 `storedPosRoundMic`/`miDragable`/`miShowClock`）**重开壁纸不复位**、且同壁纸多脚本能互相看见时；不做 A/B 对照 | ⚠ **打开后脚本会往宿主 `localStorage` 写数据**（这是本项唯一要用户拍板的纪律变更；缺省保持 legacy 就是为了不替用户决定）。回退 = 去掉 `?scriptstore=persist`（已写入的 `mpw.<包id>.*` 键会留在浏览器里，需手工清）。⚠ `?scriptcache=0` 时缓存没了但存储照旧（一条 `?scriptstore=persist` 就够） | elysia/scene-scripts.js:940 |
...
| `mpwhardmute` | `redirect`（顶层）/ `1`（帧内） | 关掉总线接管 | **插件侧**的全总线级静音档位(dsh-mpkg-wallpaper 的 lib/audio-bus.js)：off 只归因不动声音；redirect **只重定向不静音**（诊断档：验证总线有没有把所有出口收进来 —— 还能听见=有节点绕过 connect，声音变闷=有节点被重复挂 master）；1 = 重定向 + master 归零 + 媒体压制 + speechSynthesis.cancel() + WebRTC track.enabled=false；all 再加跨帧 postMessage 与"未归因 context 也压"；report 只汇报（不可控清单）。**缺省**：顶层 redirect（只归因，不顺手压宿主自己的提示音）、**帧内 1**（壁纸帧是真要压住的那一半）。可查面 window.__mpwAudioBus.report()（contexts/masters/redirects/bypassSuspected/uncontrollableFrames/log） |
| `audioart` | 缺省**豁免** | 隐藏（回到旧行为） | **音频响应型美术层**（`Audio bar(s)` / `…Spectrum Visualizer`）是否豁免两条隐藏启发式（P-170，2026-09-21）：`hideUI` 的名字正则里含 `Audio|音频|Spectrum|播放|音量|sound`，`hideBars` 又把『父组纯色遮罩条』整类关掉 —— 而作者做的可视化条**自己就是** `models/util/solidlayer.json` 的实体遮罩层 ⇒ 点名的 `Audio bar` 层被自家规则吞掉（离线逐层审计读数 `#3 Audio bar vis=0`）。现在按『名字命中音频美术词 **且** 有特效/粒子/作者属性绑定』豁免；`?audioart=hide` 关掉豁免，用于对照『这一层到底是谁在显示/隐藏』 | 怀疑『音频条/频谱条没显示』时：缺省应可见；带 `?audioart=hide` 后消失 ⇒ 证明可见性来自这条豁免 | `=hide` 时与改动前逐位一致；纯播放器外壳（`Song Title`/`Artist Name`/`Play Icon`/`.mp3`/`----MUSIC PLAYER----`）**任何档都不显示** | core/we-scene-bundle.js 的 `audioArtIds()`、demo.html 的 `hideAudioArt` |
| `texwrap` | `clamp` | repeat（名单内平铺） | **纹理 wrap 采样档**（P-168，2026-09-20；上游 1.3.18 的 REPEAT 契约）：云/神光类效果链的 uv 随 `g_Time` **无界增长**（`uv = uv0 + t*speed`），CLAMP 下超出 `[0,1]` 的部分被"拉边" ⇒ 云密度恒等、天空退化成**静止伪影**。缺省按**名单**（`core/we-scene-bundle.js` 的 `REPEAT_TEX_NAMES`，当前仅 `util/clouds_256`）对该类贴图用 `gl.REPEAT`，其余一律 `CLAMP_TO_EDGE`（图集/角色贴图改 REPEAT 会在边缘出现一圈复制）。`?texwrap=clamp` = 全部按旧行为（逐位 A/B 对照）；`?texwrap=repeat` = 全部 REPEAT（排障用） | 怀疑"云/神光贴图被拉边、天空是一层静止伪影"时；`?texwrap=clamp` 与缺省对照即可判定这一档的影响面 | `=clamp` 时与改动前逐位一致（名单不生效）；`=repeat` 时角色/UI 图集会在边缘出现复制（**只为排障**，不是可用档） | demo.html:1703（`wrapTex`）、core/we-scene-bundle.js:12264（`texWrapMode`/`applyTexWrap`） |
...
| `overbright` | `legacy` | `official` | **粒子材质常量 `ui_editor_properties_overbright`（缺省 1）的实例亮度档位**（P-149，上游 `oneincase/webwallgl` 1.3.x 的 `overbright`；本项按**规格独立实现**，登记见 `THIRD-PARTY.md` §16 + `docs/COPYING-RULES.md` §4 #14）：`official` = 材质 `passes[0].constantshadervalues.ui_editor_properties_overbright` **乘在精灵实例 RGB 上（不动 alpha）**；取值契约唯一实现在 `core/we-scene-bundle.js::particleOverbrightFactor` —— **键缺失/脏值 ⇒ 1**（`Number(null)===0`、`Number(undefined)===NaN`，直接 `Number()` 会把"没写这个键"当成 0 = 整层全黑）、负数 ⇒ 0（`Math.max(0,·)`，0 是合法值）、**不设上界**（钳了 `overbright=5` 就等于没修）。宿主 `demo.html` 在粒子材质段把因子写进 `layer.__particleOverbright`（子系各自写进 `resolveChildDefs` 的 map 条目，**不继承父层**），渲染端在实例色装配处乘进三通道（整批同色走 `u_Color` 上提、逐粒子色走 `a_Color` 顶点缓冲，两条路同一因子）。`legacy` = **恒 1**：三个颜色分量与改动前逐位相同（= 这个键被静默丢弃的旧画面）。语料实测：**38 条材质带该键 / 15 个包 / 54 个粒子层**，其中**非 1 的层 25 个**（最大 `5×`、最小 `0.17×`；`dd/3719111841` 的 Bokeh Hex/Cir = **0.25** ⇒ 今天亮 4 倍糊屏，与上游 `3151551777` 的病一模一样）| ① 真机逐层对拍官方 `$MPW_ROOT/allwallpaper/**/preview.gif` 前先看"键被忽略"的旧画面（`?overbright=legacy`）；② 某层亮度整体偏亮/偏暗（尤其 additive 大光斑糊屏、星星/玻璃碎片发暗 5 倍）时做单变量 A/B | `legacy` **逐位可回退**（门禁 `particle-overbright` ⑤：值 = 1 的层两档整条 sha256 相同、因子 ≠ 1 的层两档 sha256 **不同**且 legacy 档稳定可复现）；因子 1 的层（29/54 层、13/38 材质）在官方档下与改动前**逐字节相同**（`x*1 === x`）。⚠ 只影响粒子层的**实例色**：不动几何（位置/尺寸/UV/alpha）、不动 RNG/粒子数、不动层投影与合成主干 | core/we-scene-bundle.js:7605 |
