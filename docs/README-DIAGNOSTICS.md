...
| `projy` | `legacy` | fix（世界 y=0 → 屏幕顶） | **相机投影 y 轴口径**（P-69 用户第 1 项根因）：`legacy` = 旧的镜像口径（世界 y=0 → NDC −1 = 屏幕底），所有走 `viewProj` 的**四边形层/粒子层绕屏幕水平中线镜像**（真机 3554161528 花朵 1640.55→519.45 画到上半屏；钢琴 1053→1107 只差 54px 所以"只有花朵看着不对"）。不写 = 修正口径，与 `MESH_VERT`（蒙皮层，本来就是 y-down）、`preview.mjs` 的 `orthoYDown` 同式 | 怀疑"某四边形层位置上下不对 / 整屏像翻过来"时；对照旧口径复现 bug | ⚠ **默认值与 `qflip` 耦合**：`projy` 不写(fix) ⇒ `qflip` 默认**关**；`?projy=legacy` ⇒ `qflip` 默认**开**（改 `qflip` 默认值时必须一起想，见 PATCHES P-69 的误诊说明）。`legacy` 与改动前逐位一致，画面退回镜像构图 | core/we-scene-bundle.js:2262 |
...
| `cursor` | `off` | 开（按官方 `controlpoint[].flags:1` = lockToPointer 语义） | **指针锁定发射器**（P-69 用户第 6 项）：`?cursor=off` = 所有 lockToPointer 发射器不发射（与 `?noparticles` 全局关不同，只关这一类）。不开时：`window.__mpwPointer={x,y,inside}`（**设计坐标** 0..3840/0..2160，y 向下；宿主/测试注入，优先）或画布 `pointermove`（按画布归一化 0..1 × 相机 framed 窗口换算）→ 发射基准点=指针；**没有指针信息就不发射**（官方 3554161528 截图画面正中无放射花瓣爆，我们旧实现把 id389 的发射器退化到 authored origin=画布正中 (1920,1080) ⇒ 永久中心爆 + 9 万次/帧）。**P-118 把两条通道的优先规则说全**（每条都有 `tests/pointer-leave-test.mjs` 断言 + 反向变异）：① DOM 通道**自举** —— 建渲染器即装画布钩子（不再要求"本帧已经有指针"；否则出货页面 `demo.html`（全仓库 `__mpwPointer` 零生产者）里这类发射器**一次都不发射**）；② 画布 `pointerleave` 之后注入值**没变** ⇒ 停发（宿主喂的还是"离开前那份旧坐标"，不许盖过"人真的把鼠标移出画布"；注入值变了 = 新证据 ⇒ 认注入）；③ 注入 `inside:false` = 宿主**显式**声明"指针不在画布内" ⇒ 停发，**不**回落到上一次画布坐标 | 画面上多出一个"跟着鼠标/停在屏幕中间"的粒子源；用户要立刻消灭中心爆时用 `?cursor=off` | 关掉后该层完全不发射（作者本意的鼠标拖尾也没了，属预期；且 `?cursor=off` 下**连画布钩子都不装**，D4b 钉住）；`flags` 位定义未见官方文档，按 `flags:1` 推断（PATCHES P-69 标未定） | core/we-scene-bundle.js:6666、core/we-scene-bundle.js:6686、core/we-scene-bundle.js:6691 |
...
| `bandfeed` | `1`/`on`/`true`（有真源用真源，否则模拟）/ `sim`（只用确定性模拟源）/ `real`（只认真实源：无源 ⇒ 全零 + `silent:true`）/ `0`·`off`·空·非法 | **关** | **128 元频段数组接线**（P-112-BANDGEOM，任务书 §5-4 / P1-5）：把 `core/audio-band-array.mjs`（规格 `docs/AUDIO-BAND-SPEC.md`）接进两条消费路径 —— ① 场景层脚本的 `registerAudioBuffers`（`audioBuffers(n)` 改为从 128 元数组重采样，与网页壁纸同口径）；② 宿主（父页）按 20Hz 节流收 `{type:'mpw-audio-bands', len:128, source, t, bands}`。真实源 = 渲染器自己的 `AnalyserNode`（包内 sound 层，需 `?audio=1` 且真的在播）⇒ `clampOnly`（不套 γ）；无真实源 ⇒ 确定性模拟源（γ=1.8/gain=1.8）。诊断：`window.__mpwAudioBands` / `__mpwAudioBandStats()`（`silent` = 到底接上没有）/ `__mpwAudioBandSource` | 排查"音条不动"时先看 `silent` 与 `source`：区分"**没有数据源**"与"**数据源很安静**"（处置完全不同）；`sim` 做单变量 A/B；`real` 验"无源不报错、不阻塞"（P1-5 验收口径） | ⚠ **本机没有系统声卡环回** ⇒ 默认的"真实源"是**包内音频**，不是系统音乐；渲染器 → 宿主的消息**契约未确认**（in-repo 无消费者；插件侧的频谱桥是父页 → 帧内 `op:'audio'`）⇒ 只是"缺的那一半"的提案，见 `docs/AUDIO-BAND-WIRING.md` §4。关时零 `postMessage`、零新字段 | demo.html:2509 |
