...
> 全部改动集中于 `core/we-scene-bundle.js`；`demo.html` 本轮**无需改动**（P0 均在渲染器内核）。
> 服务器（server/we-scene-demo-server.mjs）按请求读盘返回 bundle/demo，改动即生效，**未重启任何服务**。
...
| 验证项 | 命令 | 结果 |
|---|---|---|
| 语法 | `node --check core/we-scene-bundle.js` | ✅ OK（每条改动后各跑一次） |
...
- **位置**：`core/we-scene-bundle.js:3463`（`const binds = ((ov && ov.bind) || mp.binds || [])`，随后的 `for (const b of binds)` 替换原 `for (const b of eff.binds || [])`）。
...
- **位置**：`core/we-scene-bundle.js:2870-2915`（`progVAO` + `fxVao` + 重写 `bindVAOFor`）、`3449`（pass 循环 `gl.bindVertexArray(fxVao)`，原为 `vao`）。
...
- **位置**：`core/we-scene-bundle.js:3078`（原 `const parEnabled = …` → `parEnabled = …` 赋值外层闭包变量）。
...
- **位置**：`core/we-scene-bundle.js:2976`（`gl.blendFuncSeparate(gl.SRC_ALPHA, gl.ONE, gl.SRC_ALPHA, gl.ONE)`，原 alpha 通道为 `(ONE, ONE)`）。
...
- **位置**：`core/we-scene-bundle.js:3476-3486`（纹理绑定循环内，`entry` 解析后：`if (entry.fbo && outFBO && entry.fbo === outFBO.fbo) entry = passInput` + 一次性日志）。
...
- **位置**：
  - `core/we-scene-bundle.js:2498-2523`（PARTICLE_VS 增 `in float a_Alpha; out float v_Alpha;`；PARTICLE_FS 增 `in float v_Alpha`，alpha = `u_Alpha * v_Alpha * texR`，对齐官方 genericparticle "alpha=v_Color.a" 语义）；
  - `core/we-scene-bundle.js:2605-2620`（新增专用 `partVao`：stride 24 = pos3+uv2+alpha1，`partAlphaLoc = getAttribLocation(particleProg,'a_Alpha')` 守卫 ≥0）；
  - `core/we-scene-bundle.js:3583-3613`（顶点缓冲扩为 6 float/顶点，`verts[vi++] = a` 写入逐粒子 alpha，绘制绑定 partVao）。
