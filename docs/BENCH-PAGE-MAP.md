...
| | ↑ ⑧(2026-09-19 品牌改名) **品牌名是运行期覆盖的**：patch `applySiteBrandNow()` 把 `.brand-name`（选择器 `[data-i18n="app.title"]`）与 `document.title` 写成产品现名 **WEwebLoader**，版本号 `#app-version` 不碰；`?appname=upstream` / `?brand=upstream` 还原上游名 `wallpaper-engine-webgl`。静态 `<title>`/DICT/minified 产物**不动**（分别是 gate T8 / T1 / 许可口径的钉子） | — | gate **T5b**（纯函数 + 假 DOM 冷启动 + 语言切换后重放） |
