...
async function renderCurrent(query, outFile) {
  ...
  console.log(`渲染: ${url}`)
  execFileSync('node', ['headless-shot.mjs', url, outFile, '8000', String(RENDER_W), String(RENDER_H)],
    { cwd: import.meta.dirname, stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env, REFR: '0', SHOT_MODE: process.env.SHOT_MODE || 'cpu' } })
...
function geometricScore(id) {
  const rr = path.join(import.meta.dirname, 'refrender-' + id + '.json')
  if (!fs.existsSync(rr)) return null
  try {
    const out = execFileSync('node', ['layer-rect-check.mjs', id, '--refrender'], { cwd: import.meta.dirname, encoding: 'utf8', timeout: 120_000 })
...
// ── 基线 ──
const BASELINE = path.join(import.meta.dirname, 'visual-baseline.json')
