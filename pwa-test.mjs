...
{
  const raw = fs.readFileSync(path.join(HERE, 'manifest.webmanifest'), 'utf8')
  let m = null
...
{
  try {
    const out = execFileSync(process.execPath, [path.join(HERE, 'make-icons.mjs'), '--check'], { cwd: HERE, encoding: 'utf8', timeout: 60000 })
...
{
  let syn = true, err = ''
  try { execFileSync(process.execPath, ['--check', path.join(HERE, 'sw.js')], { encoding: 'utf8' }) } catch (e) { syn = false; err = String(e.stderr || e.message).split('\n')[0] }
...
console.log('\n[F] 真子进程服务：PWA 资源与首页注入的实际字节')
{
  ...
  try {
    ...
    if (ready) {
      ...
      check('F9 线上 sw.js 与磁盘逐字节相同', swSrc === fs.readFileSync(path.join(HERE, 'sw.js'), 'utf8'))
      const pol = await (await fetch(base + '/sw-policy.mjs')).text()
      check('F10 线上 sw-policy.mjs 与磁盘逐字节相同（SW import 的就是它）', pol === fs.readFileSync(path.join(HERE, 'sw-policy.mjs'), 'utf8'))
    }
  } catch (e) {
