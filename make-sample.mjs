// make-sample.mjs — generate a hand-authored, copyright-clean sample Wallpaper Engine scene package.
//
...
//                                           → `node pack-dir.mjs samples/sample-synthetic-src` repacks them
...
// we-scene-bundle.js `parsePkg`, both verified below):
//   sizedString(magic = 'PKGV0022')       // i32 length + UTF-8, matches pack-dir.mjs
//   i32 entryCount
...
//
// USAGE
//   node make-sample.mjs                      # write/refresh samples/**
//   node make-sample.mjs --out <dir>          # write somewhere else
//   node make-sample.mjs --prod-verify        # also re-read scene.pkg with the PRODUCTION parser
//                                             # (dsh-mpkg-wallpaper/lib/pkg-extract.js, sha256 per entry)
//   node make-sample.mjs --pkg-extract <path> # override that parser path
//
...
const PKG_MAGIC = 'PKGV0022'                 // same magic pack-dir.mjs writes; parsePkg only checks /^PKGV\d{4}$/
 ...
 * pack-dir.mjs, which sorts the same way) produce identical bytes.
 */
...
function projectJson() {
  return {
    ...
    description: 'Hand-authored sample scene generated by make-sample.mjs. Contains no third-party content.',
    file: 'scene.pkg',
    general: {
      properties: {
...
async function main() {
  ...
  if (argv.includes('--help') || argv.includes('-h')) {
    console.log('usage: node make-sample.mjs [--out <samples dir>] [--prod-verify] [--pkg-extract <path>] [--quiet]')
    return
  }
  ...
  const outRoot = path.resolve(argOf('--out', path.join(HERE, 'samples')))
  ...
  if (argv.includes('--prod-verify')) {
    const extractPath = path.resolve(argOf('--pkg-extract', path.join(HERE, '..', 'dsh-mpkg-wallpaper', 'lib', 'pkg-extract.js')))
