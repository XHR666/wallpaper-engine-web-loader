...
Every file under `samples/` is **generated by [`../make-sample.mjs`](../make-sample.mjs)** from
...
```bash
node make-sample.mjs                 # writes samples/** — deterministic, same bytes every run
node make-sample.mjs --prod-verify   # + re-read scene.pkg with the PRODUCTION parser (sha256 per entry)
node make-sample.mjs --out /tmp/s    # write somewhere else
```
...
```bash
node --check make-sample.mjs
node make-sample.mjs --prod-verify        # parsePkg + readPkgEntry, sha256 per entry
