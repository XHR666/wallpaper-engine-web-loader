...
const README = path.join(here, 'README-DIAGNOSTICS.md')
const JSON_OUT = path.join(here, 'diag-flags.json')
const CLIENT = path.join(ROOT, 'dsh-mpkg-wallpaper', 'lib', 'client.js')
...
function collect() {
  ...
  absorb('we-scene-bundle.js', fs.readFileSync(path.join(here, 'we-scene-bundle.js'), 'utf8'))
  absorb('demo.html', fs.readFileSync(path.join(here, 'demo.html'), 'utf8'))
  const elysiaDir = path.join(here, 'elysia')
  const walk = (dir) => {
