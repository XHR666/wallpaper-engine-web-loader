...
// [browser] node fs 移除; 全局 shader 经注入的 weAssetsRead(name)->bytes 读取
...
export function installGlsl(proto) {
  ...
  proto._compileWorkshopEffect = function (ef) {
    ...
    if (!frag) {
      ...
      if (this.weAssetsRead) {
        const b = this.weAssetsRead('assets/effects/' + name + '/shaders/effects/' + name + '.frag');
        if (b != null) frag = new TextDecoder().decode(b);
      }
    }
    if (!frag) { this._glslCache.set(key, null); return null; }
    let vert = '';
    try { vert = this.pkg.readText(stem + '.vert') || ''; } catch {}
    if (!vert && this.weAssetsRead) {
      const b = this.weAssetsRead('assets/effects/' + name + '/shaders/effects/' + name + '.vert');
      if (b != null) vert = new TextDecoder().decode(b);
    }
  ...
  proto._resolveGlslInclude = function (inc) {
    if (this.weAssetsRead) {
      const b = this.weAssetsRead('assets/shaders/' + inc);
      if (b != null) return new TextDecoder().decode(b);
    }
