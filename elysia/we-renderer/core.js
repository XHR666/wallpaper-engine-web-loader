...
// [browser 版] node fs/path 移除:
//   - pkg 由注入的 read/readJson 接口提供 (来自 we-scene-bundle parsePkg/getEntry)
//   - 纹理解码走本包 textures.js (parseTex/decodeMip0)
//   - Buffer 用 ../buffer.js 的浏览器 shim
...
import { loadTexImage, loadPngFile } from './textures.js';
...
export class SceneRenderer {
  constructor(pkg, opts = {}) {
    // [browser] pkg is an in-memory accessor (must expose read/readJson/has).
    //   { has, read(name)->Uint8Array|null, readJson(name)->object|null, readText(name)->string|null }
    this.pkg = pkg;
    this.pkgPath = null;
    this.log = opts.log || (() => {});
    ...
    // [browser] weAssetsDir 是 node 路径 (无 fs); 改用注入的 weAssetsRead(name)→bytes|null
    this.weAssetsDir = opts.weAssetsDir || null;
    this.weAssetsRead = opts.weAssetsRead || null;
    // 视差鼠标位置 (0-1, 默认中心)
  ...
  readJsonAny(rel) {
    ...
    if (j) return j;
    // [browser] 全局资产经注入的 weAssetsRead(name) 读取 (name 形如 assets/materials/...) 
    if (this.weAssetsRead) {
      try {
        const b = this.weAssetsRead('assets/' + rel);
        if (b != null) return JSON.parse(new TextDecoder().decode(b));
      } catch { /* ignore */ }
      return null;
    }
    return null;
  ...
  readAny(rel) {
    ...
    if (b) return b;
    if (this.weAssetsRead) {
      try { return this.weAssetsRead('assets/' + rel); } catch { /* ignore */ }
    }
    return null;
  }
  ...
  loadTexture(pathOrName) {
    ...
    // [browser] 经注入的 weAssetsRead(name) 读取 (name 形如 assets/materials/...)
    if (!img && this.weAssetsRead) {
      try {
        const gRaw = this.weAssetsRead('assets/' + texPath);
        if (gRaw != null) img = loadTexImage(gRaw);
      } catch (e) {
        this.log('全局纹理解析失败 ' + texPath + ': ' + e.message);
      }
    } else if (!img && this.weAssetsDir) {
      const globalPath = this.weAssetsDir + '/assets/' + texPath;
      this.log('browser 模式无 fs, 跳过全局资产路径 ' + globalPath);
    }
  ...
  // [browser] videoFrames 项可为 {width,height,rgba} 或 PNG 字节(Uint8Array)
  _readVideoFramePng(ref, pngPath) {
    const png = pngPath || (this.videoFrames && this.videoFrames[ref]);
    if (!png) return null;
    try {
      if (typeof png === 'object' && png.rgba) return { width: png.width, height: png.height, rgba: png.rgba };
  ...
  _loadAssetPng(texPath) {
    ...
    const f = map[name];
    if (!f) return null;
    // [browser] assetDir 可为注入的 read(name)->bytes; 否则 node 路径模式(不可用)
    if (typeof this.assetDir === 'function') {
      try {
        const b = this.assetDir(f);
        return b ? loadPngFile(b) : null;
      } catch (e) { return null; }
    }
    return null;
  }
