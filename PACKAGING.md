...
> 本文件是这些动作的**唯一的操作与自证手册**；渲染能力本身见 `RENDERER-ARCHITECTURE.md`，逐层调试开关见
> `README-DIAGNOSTICS.md`，对外介绍见 `README-PUBLIC.md`。
>
...
| 命令 | 干什么 | 退出码 |
|---|---|---|
| `bash start-demo.sh` | 预检（node 版本 / 包解析器落点 / 语料 / 样例 / 端口）+ 起 `we-scene-demo-server.mjs` | 0 正常；2 用法错；3 预检失败 |
...
| `node we-scene.mjs` | 不是 CLI；这是**库入口**（见 §3） | — |
...
解析链与 `we-scene-demo-server.mjs` **逐条同序**（`start-demo.sh --check` 用的是同一顺序）：
...
| `version` | `0.1.0` | 与 `we-scene.mjs` 的 `VERSION` **必须一致**（`mount-test.mjs` 断言） |
...
| `exports` | `.` → `we-scene.mjs`、`./bundle`、`./server`、`./hlsl2glsl`、`./package.json` | 每条目标都真实存在（断言过） |
...
```
117 文件 · tarball 1.9MB · 解包 4.9MB · 最大单文件 we-scene-bundle.js 506.5kB
打包清单含：we-scene.mjs / manifest.webmanifest / sw.js / sw-policy.mjs / icons/*.png /
...
`we-scene-bundle.js`（506KB，141 个导出）就是"单文件 ESM"，浏览器直接
...
| 开关 | 作用 |
|---|---|
| `MPW_PWA=1 node we-scene-demo-server.mjs` | 该服务器上**所有**首页都注入（部署一次，推荐） |
| `http://…/?pwa=1` / `?pwa=0` | 单次请求覆盖环境变量（临时试用 / 明确关闭） |

默认关的理由：它会写 Cache Storage。只有明确要"装成应用 / 离线可用"的部署才该打开。

> 这是**服务器侧**开关：`diag-flag-check.mjs` 的抓取源是 `we-scene-bundle.js` / `demo.html` /
> `elysia/**/*.js` / 插件 `lib/client.js`，**不含** `we-scene-demo-server.mjs` ⇒ README-DIAGNOSTICS 的
...
| 文件 | 作用 |
|---|---|
| `manifest.webmanifest` | `start_url = /?id=sample-synthetic`（**离线打开就有画面**）、`display: standalone`、3 个图标（含 maskable） |
| `sw.js` | **module** service worker：网络优先 + 缓存兜底；`activate` 清旧版本缓存 |
| `sw-policy.mjs` | **缓存判据**（纯函数、单一事实源）：`shouldCache()` / `shouldStoreResponse()` |
| `pwa-inject.mjs` | 首页注入（幂等；缺 `</head>` 原样返回）+ PWA 静态路由表 |
| `icons/` | 程序化生成的图标（`node make-icons.mjs`，确定性、无第三方素材）+ `icons/icons.json` 的 sha256 台账 |
...
| 4 | **hlsl2glsl 接入可选效果路径** | ⚪ vendored + 覆盖率门禁已做（P-93），但在 `we-scene-bundle.js` 里**接线未做**（该文件由并发线独占） |
| 5 | **type 声明（`.d.ts`）** | ⚪ 无。`mount()` 的契约在 `mount-test.mjs` 里以断言形式存在 |
| 6 | **tarball 的 `files` 白名单** | ⚠ 我（P-91）只加了 `sw-policy.mjs`、移出 `check.sh`；其余条目由建仓线维护，若两边同时改同一字段会互相覆盖 |
